  ...
  <div id="cv" style="background-color: #f7f1ed; padding: 5%">
    <div class="header">
      ...
      <?php else: ?>
        ...
        <img src="https://randomuser.me/api/portraits/<?= rand(0, 1) ? 'women' : 'men' ?>/<?= rand(1, 99) ?>.jpg" alt="Photo de <?= htmlspecialchars($data['infos-perso']['firstname'] . ' ' . $data['infos-perso']['lastname']) ?>">
      <?php endif; ?>
 ...
 -->
        ...
        <div class="section competence">
          ...
          foreach ($competences_perso as $key => $competence) :
            if (trim($competence['competence']) !== '') :
              if (trim($competence['niveau']) !== '') : ?>
                <li><span><?= $competence['competence'] ?></span> - <span><?= $competence['niveau'] ?></span></li>
              <?php else : ?>
                <li><span><?= $competence['competence'] ?></span></li>
            <?php endif;
            endif; ?>
          <?php endforeach;
          ?>
  ...
  <div id="download">
    ...
    <script>
      document.getElementById('btn-telecharger').addEventListener('click', function() {
        ...
        window.location.href = 'generated_cv_pdf.php';
      });
    </script>
